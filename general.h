...
unsigned long long FastRandom_rand(FastRandom *t) {
  t->rnd ^= t->rnd << 21;
  t->rnd ^= t->rnd >> 35;
  t->rnd ^= t->rnd << 4;
  return t->rnd;
...
char GLOBAL_EXPECTATION_FLAG = OK;

#define EXPECTATION_FAILED() do {GLOBAL_EXPECTATION_FLAG = NOT_OK;} while (0)

#define EXPECT_TRUE(expr) do {if (!(expr)) EXPECTATION_FAILED();} while (0)
#define EXPECT_FALSE(expr) do {if ( (expr)) EXPECTATION_FAILED();} while (0)

#define EXPECT_EQ(expr1, expr2) do {if ((expr1) != (expr2)) EXPECTATION_FAILED();} while (0)
